...
main(int argc, char **argv)
{
	...
	/* show size of ucontext_t structure. Hint: use sizeof(). */
        long int ucontext_t_size=sizeof(ucontext_t);
	printf("ucontext_t size = %ld bytes\n", (long int)ucontext_t_size);
	...
	printf("memory address of main() = 0x%lx\n", (unsigned long)&main);
        ...
        unsigned long RIP_adddess=mycontext.uc_mcontext.gregs[REG_RIP];
	printf("memory address of the program counter (RIP) saved "
	       "in mycontext = 0x%lx\n",
	       (unsigned long)RIP_adddess);

	/* now, think about parameters */
        int argc_address=mycontext.uc_mcontext.gregs[REG_RDI];
	printf("argc = %d\n",argc_address );//not sure why EDI doesn't work
